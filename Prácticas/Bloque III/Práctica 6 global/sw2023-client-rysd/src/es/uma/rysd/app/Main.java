...
public class Main {
	...
	public static void main(String[] args) throws IOException { // Como en la clase SWClient hemos usado throws en lugar
																// de try/catch, debemos implementarlo aqu� tambien (en
																// todos los m�todos que lo utilicen)
		// Descomente las siguiente l�neas si lo est� probando en el laboratorio y
		// accede a Internet usando el proxy
	...
	// Genera un n�mero entre 0 y max-1 que no haya sido usado previamente (los
	// usados vienen en l)
	...
	// Pregunta que obtiene dos recursos iguales (personajes en este caso) y los
	// compara
	public static void tallest(SWClient sw) throws IOException {
		...
		Integer n = null;
		do {
			System.out.println("¿Quien es mas alto? [0] " + people.get(0).name + " o [1] " + people.get(1).name);
			try {
	...
	// - Pregunta sobre alg�n recurso relacionado (persona que naci� o fue crear
	// ah�)
	// - Busca ese recurso y comprueba si est� relacionado con el primero (si la
	// persona buscada tiene como planeta el original)
	...
	// Similar a la previa pero en vez de solicitar que escriba se le ofrecen
	// alternativa para ello:
	// - Se elige una al azar de las disponibles en el recurso, persona del planeta
	// (la correcta)
	// - Se buscar aleatoriamente otras 3 que no est�n relacionados con el recurso
	// (las incorrectas)
	...
	public static void filmIn(SWClient sw) throws IOException {

		// Obteniendo la cantidad de pel�culas y personas
		int max_people = sw.getNumberOfResources("people");
		int max_film = sw.getNumberOfResources("films");
		if (max_people == 0 || max_film == 0) {
			System.out.println("No se encontraron personas o peliculas.");
			return;
		}

		System.out.println("Generando nueva pregunta...");
		// Obteniendo la pel�cula (con personas)
		List<Integer> used = new ArrayList<Integer>();
		Movie film = null;
		do {
			Integer p = getRandomResource(max_film, used);
			film = sw.getMovie(sw.generateEndpoint("films", p));
			if (film == null) {
				System.out.println("Hubo un error al encontrar el recurso " + p);
			}
			used.add(p);
		...
		Integer n = null;
		do {
			System.out.print("¿Quien sale en " + film.title + "?");
			for (int i = 0; i < 4; i++) {
				System.out.print(" [" + i + "] " + people.get(i).name);
			}
			System.out.println();
		...
		// Se muestran los resultados
		for (Person p : people) {
			System.out.println(p.name + " salio en " + sw.getMovie(p.films[0]));
		}
	...
	private static String[] success = { "This is the way", "Eres uno con la Fuerza. La Fuerza esta contigo",
			"Que la fuerza te acompañe", "Nada ocurre por accidente", "Sin duda, maravillosa tu mente es",
			"Cuando te fuiste, no era mas que el aprendiz. Ahora eres el maestro",
			"La Fuerza te esta llamando, dejala entrar", "Tu cantidad de midiclorianos debe ser muy alta",
			"Una leccion aprendida es una leccion ganada", "Debes creer en ti mismo o nadie lo hara",
	...
	private static String[] error = { "El mejor profesor, el fracaso es.",
			...
			"Tu carencia de fe resulta molesta", "La capacidad de hablar no te hace inteligente",
			"Concentrate en el momento. Siente, no pienses, usa tu instinto",
			"No lo intentes. Hazlo, o no lo hagas, pero no lo intentes", "Paciencia, utiliza la Fuerza. Piensa",
			"Siento una perturbacion en la fuerza", "El lado oscurso se intensifica en ti",
